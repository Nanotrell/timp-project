#!/bin/bash
# ============================================================================
# ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ: плавный график, таблица 20 точек, равные оси
# ============================================================================
...
# ============================================================================
# 1. ИСПРАВЛЯЕМ math_engine.cpp (200 точек для графика, 20 для таблицы)
# ============================================================================
echo ""
echo "1. Исправление math_engine.cpp..."
...
# ============================================================================
# 2. ИСПРАВЛЯЕМ postgresqlserver.cpp (отправка 200+20 точек)
# ============================================================================
echo ""
echo "2. Исправление postgresqlserver.cpp..."

cd server

cat > postgresqlserver.cpp << 'EOF'
...
# ============================================================================
# 3. ИСПРАВЛЯЕМ КЛИЕНТ (компактные ползунки, равные оси)
# ============================================================================
echo ""
echo "3. Исправление клиента..."

cd client

# mainwindow.cpp - компактные ползунки
...
MainWindow::MainWindow(const QString& login, QWidget *parent) : QMainWindow(parent), m_login(login)
{
    ...
    m_tableWidget->setMaximumHeight(180);
    ...
    logoutBtn->setStyleSheet("QPushButton { background-color: #e74c3c; color: white; font-weight: bold; border-radius: 4px; }");
...
void MainWindow::onCalcResponse()
{
    while (m_socket->bytesAvailable()) {
        ...
        if (!dataStr.trimmed().startsWith("{")) continue;
...
# plotwidget.cpp - равные оси (1:1 масштаб)
cat > plotwidget.cpp << 'EOF'
...
echo ""
echo "✅ Что исправлено:"
echo "   - График рисуется из 200 точек (плавный)"
echo "   - Таблица показывает 20 точек"
echo "   - Ползунки компактные (фиксированная ширина 250px)"
echo "   - Оси координат сбалансированы"
echo ""
echo "🚀 Запуск:"
